...
// Creating a table has no catalog to compare against, so the tests pin the
// whole statement rather than a diff: every field, the primary key and every
// index are written out of the draft.

func newTableDraft() models.TableDesign {
	return models.TableDesign{
		...
		Columns: []models.DesignColumn{
			...
			{Name: "email", DataType: "varchar(255)", Comment: "login address"},
			{Name: "created_at", DataType: "timestamp", DefaultValue: strptr("CURRENT_TIMESTAMP")},
		},
		Indexes: []models.DesignIndex{
			{Name: "uq_customers_email", Columns: []string{"email"}, Unique: true},
		},
	}
}

func draft(t *testing.T, dialect drivers.Dialect) models.DesignPlan {
	t.Helper()
	plan, err := PlanCreate(dialect, newTableDraft())
	if err != nil {
...
func TestPlanCreateMySQL(t *testing.T) {
	plan := draft(t, MySQLDialect{})
	if plan.Destructive {
		t.Error("creating a table does not drop anything")
	}
	assertStatements(t, plan.Statements, []string{
		"CREATE TABLE `shop`.`customers` (\n" +
			...
			"  PRIMARY KEY (`id`)\n" +
			")",
		"CREATE UNIQUE INDEX `uq_customers_email` ON `shop`.`customers` (`email`)",
	})
...
func TestPlanCreateTiDBSharesTheMySQLPlan(t *testing.T) {
	assertStatements(t, draft(t, MySQLDialect{Driver: models.DriverTiDB}).Statements,
		draft(t, MySQLDialect{}).Statements)
}

func TestPlanCreatePostgres(t *testing.T) {
	plan := draft(t, PostgresDialect{})
	assertStatements(t, plan.Statements, []string{
		"CREATE TABLE \"public\".\"customers\" (\n" +
			...
			"  PRIMARY KEY (\"id\")\n" +
			")",
		// PostgreSQL keeps comments beside the table, not on the column.
		"COMMENT ON COLUMN \"public\".\"customers\".\"email\" IS 'login address'",
		"CREATE UNIQUE INDEX \"public\".\"uq_customers_email\" ON \"public\".\"customers\" (\"email\")",
	})
}

func TestPlanCreateSQLiteIsHonestAboutItsLimits(t *testing.T) {
	// SQLite's default database is `main`, and its dialect leaves that name out
	// of qualified names because that is how its own DDL reads.
	sqlite := newTableDraft()
	sqlite.Database = "main"

	plan, err := PlanCreate(SQLiteDialect{}, sqlite)
	if err != nil {
		t.Fatalf("PlanCreate: %v", err)
	}
	assertStatements(t, plan.Statements, []string{
	...
	if len(plan.Warnings) != 1 || !strings.Contains(plan.Warnings[0], "no column comments") {
		t.Fatalf("expected the comment to be reported as unsupported, got %v", plan.Warnings)
	}
}

func TestPlanCreateWarnsAboutAutoIncrementWithoutAKey(t *testing.T) {
	// MySQL only accepts AUTO_INCREMENT on a column that starts a key, so the
	// designer says so instead of writing a statement the engine will reject.
	mysql := newTableDraft()
	mysql.Columns = []models.DesignColumn{
		{Name: "id", DataType: "bigint", PrimaryKey: true},
		{Name: "seq", DataType: "bigint", AutoIncrement: true},
	}
	mysql.Indexes = nil
	plan, err := PlanCreate(MySQLDialect{}, mysql)
	if err != nil {
		t.Fatalf("PlanCreate: %v", err)
	}
	if len(plan.Warnings) != 1 || !strings.Contains(plan.Warnings[0], "AUTO_INCREMENT") {
		t.Fatalf("expected a warning about the unkeyed AUTO_INCREMENT, got %v", plan.Warnings)
	}

	// SQLite auto-increments its INTEGER PRIMARY KEY by definition, so the flag
	// on any other column is a no-op worth reporting.
	sqlite := newTableDraft()
	sqlite.Database = "main"
	sqlite.Indexes = nil
	sqlite.Columns = []models.DesignColumn{
		{Name: "id", DataType: "integer", PrimaryKey: true, AutoIncrement: true},
		{Name: "seq", DataType: "integer", AutoIncrement: true},
	}
	plan, err = PlanCreate(SQLiteDialect{}, sqlite)
	if err != nil {
		t.Fatalf("PlanCreate: %v", err)
	}
	if len(plan.Warnings) != 1 || !strings.Contains(plan.Warnings[0], "INTEGER PRIMARY KEY") {
		t.Fatalf("expected a warning about the AUTOINCREMENT that does nothing, got %v", plan.Warnings)
	}
}

func TestPlanCreateNeedsANameAnEngineAndAField(t *testing.T) {
	nameless := newTableDraft()
	nameless.Object = "  "
	if _, err := PlanCreate(MySQLDialect{}, nameless); err == nil {
		t.Fatal("expected a table without a name to be refused")
	}

	fieldless := newTableDraft()
	fieldless.Columns = nil
	if _, err := PlanCreate(MySQLDialect{}, fieldless); err == nil {
		t.Fatal("expected a table without fields to be refused")
	}

	if _, err := PlanCreate(OracleDialect{}, newTableDraft()); err == nil {
		t.Fatal("expected the designer to refuse an engine it cannot create tables on")
	}
}
