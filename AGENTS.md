...
```sh
just typecheck                 # 前端 tsc --noEmit（含 antd API 校验）
just test                      # go test ./...
git add -A
...
```sh
just sync "feat(sidebar): 索引成为一等资源"
```
...
发版 = 改版本号 + 打 tag + push，剩下的交给 GitHub Actions。

```sh
just release 0.2.0 "新增 Navicat 风格连接树；索引成为一等资源；支持 PostgreSQL 分区表"
```
...
1. **附注 tag 的 message** —— 也就是 `just release` 的第二个参数，人写的本版总结；
...
| `Justfile` | `version := "…"` | `just build` 的 ldflags 来源 |
| `app.go` | `var Version = "…-dev"` | 开发期兜底；发布构建用 ldflags 覆盖 |

`just check-version`（CI 里也跑）会在四处不一致时报错。
**不要手改这些字段**，一律走 `just release` 或 `node scripts/version.mjs <x.y.z>`。
...
- `wails`、`just`、`git`（推送）要用 Windows 侧的可执行文件，**不要在 WSL 里直接跑**：
...
- 构建用的是 Windows 工具链，所以 `just build` / `wails build` 必须在 Windows 侧执行。
