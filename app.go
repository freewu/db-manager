...
// PlanRowDelete renders the statement one row of a selection is about to be
// removed by, so the grid can list them before the user agrees to it. Nothing is
// executed.
func (a *App) PlanRowDelete(req models.RowDelete) (string, error) {
	return a.manager.PlanRowDelete(req)
}
...
// PlanInsertRows renders the statement the data generation window is about to
// run, so the window can show it before the user agrees to it. The values are
// made up while the run goes on, so what it shows is the statement's shape.
func (a *App) PlanInsertRows(req models.RowInsert) (string, error) {
	return a.manager.PlanInsertRows(req)
}
