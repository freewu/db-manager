...
│   │   └── all/              # 汇总导入，保证 init 注册
│   └── service/manager.go    # 会话管理、超时、只读校验、审计入口（compare.go：两个库的比较与同步脚本；export.go：整库 / 整表的导出，边读边写）
└── frontend/
    └── src/
