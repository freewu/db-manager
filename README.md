...
│   │   ├── sqlbase/          # 通用 database/sql 实现：连接池、分页、脚本执行、DDL（design.go 的规划器 + copy.go 的复制表 + tableops.go 的清空 / 删除 + diff.go 的两张结构对比）、行变更（含 literal.go 的语句预览字面量）、运行情况外壳
...
│   │   └── all/              # 汇总导入，保证 init 注册
│   └── service/manager.go    # 会话管理、超时、只读校验、审计入口（compare.go：两个库的比较与同步脚本）
└── frontend/
    └── src/
