...
└── frontend/
    └── src/
        ...
        │   ├── SettingsDialog.tsx # 设置窗口：外观 / 代码生成 / Mock 占位符 / 数据目录（含日志轮转阈值）/ 关于
...
收起那一刻宽度就不再有地方放了，所以要让树回到比这更宽，得再拖一次。

命令条上的 **Change Log** 与 **Data Generation** 点开时会**顺手把树收起来** —— 这两页都是「左右两栏、要宽度」
的窗口（变更日志左边列表右边语句，数据生成左边挑表右边一整张字段表），而且都是**先把表挑好了才打开**的，
树在那一刻帮不上忙；从表右键打开数据生成**不会**收起（右键就发生在树里，收起来等于把刚点的东西藏了）。
...
`<数据目录>/changelog.json` 的 `maxEntries` 里，设置窗口的 `Data folder` 页改它）就**整份改名**成
...
- [x] Phase 1.5：
  ...
  - [x] 变更日志：命令条 `Change Log` 一页看这个程序跑过的每一句写语句（时间 / 连接 / 库 / 表 / 语句），存在数据目录里，没连库也能打开；写满了就整份留档成 `20260214-1.log`（阈值可设，默认 2000 条），历史日志在同一个窗口里选着看
