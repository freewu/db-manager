...
- **行编辑**：双击单元格内联编辑、批量删除选中行，也可以在**行详情里一次改多个字段**（改完先看后端渲染出的那条 `UPDATE`，确认才执行 —— 预览与真正执行的语句由同一段渲染代码产出，所以确认框里就是发出去的那条）；所有写操作都以主键为条件、**全部使用参数绑定**，删行同样先渲染出 `DELETE` 再执行 —— 一次删多行就把每一行的 `DELETE` 都摆出来（一行一条、按将要执行的顺序，删几行一眼看得出来）。这些改动会进**变更日志**（来源标为 `grid`，并记下引擎报的**受影响行数**），见下。
...
- **复制表**：表右键菜单里的 `Duplicate table` 分两档 —— `Structure only`（字段、主键、索引，复制出来是一张空表）与 `Structure and data`（再加一条 `INSERT INTO … SELECT …`，行由**服务端自己搬**，一个字节都不经过本程序）。定义是从**实时目录**读的而不是窗口拼的，渲染用的还是建表那个规划器，所以预览里就是真正发出去的那几条；表名默认 `<表名>_copy`，建完直接停在副本上（带数据的停在数据页，只要结构的停在结构页）。两处**不能照抄**的地方写在警告里而不是让引擎执行到一半才报错：PostgreSQL / SQLite 的索引名归 schema（SQLite 是库）所有、原来那个名字已经被占着，副本的索引改叫 `<新表名>_<原名>`，而 MySQL 一族索引名归表所有、原样复用；PostgreSQL 的 `serial` 列默认值指向**原表那条序列**，副本改成 identity 列（数据也一起搬时再补一句：显式写进去的 id 不会推进副本自己的计数器，下一次不给主键的插入可能撞上搬来的行）。副本的每条语句都进**变更日志**（来源 `copy`），作用对象写的是新建的那张表。
- **清空表 / 删除表**：表右键菜单里的 `Truncate table` 与 `Drop table` 各自只写**一条**语句，而且**不开窗口** —— 它们没有需要用户决定的东西，后端从**实时目录**渲染出语句、在确认框里摆出来（带语法高亮），点一下就是全部交互。预览与执行仍是两次调用：真正跑的时候后端**再规划一遍**，而不是把预览的那段文本交回去。`Drop table` 是 `DROP TABLE`，**不写 `IF EXISTS`、也不写 `CASCADE`**（前者会把「已经没了」和「刚删掉」说成同一件事，后者会顺手带走依赖它的视图与约束；引擎因为有人依赖而拒绝时，它自己点名的那句话比这里瞎猜的警告更有用）；删除会**关掉这张表已打开的窗口** —— 窗口是按表命名并读它的，表没了就没什么可显示。`Truncate table` 只清行、留着表、字段和索引，各引擎的差别**写在警告里而不是执行到一半才发现**：MySQL 一族的 `TRUNCATE` 顺手把 `AUTO_INCREMENT` 归零、没有话说；PostgreSQL 的 `TRUNCATE` **不动** identity / serial 列背后的序列，于是写明「哪个字段的计数器不重置、接下来的行会接着刚删掉的号往下走」（`RESTART IDENTITY` 是另一件事，留给用户自己决定，不塞进这条语句）；SQLite 根本没有 `TRUNCATE`，发出去的是 `DELETE FROM`，连同「表还在」和「行号计数器留在原处」一起说清楚。两个入口跟建表要同一个能力（引擎能建表、且连接不是只读），且单独占一个分隔区 —— 都撤不回来，不该和「Copy name」挤在一起。两条语句都进**变更日志**（来源 `explorer`，作用对象是这张表），因为它们是树里发出去的写操作，一个显示这张表的窗口都没开 —— `Truncate` 那条还记着引擎报的**删掉了几行**。
- **数据库比较**：图标栏上的**比较**开一整页，把两个**同一种引擎**的数据库摆在一起看差异，并生成把右边改成左边的脚本。每一边自己挑连接、库、以及（有 schema 层的引擎再挑）schema，另一边那一栏就只剩下同一种引擎的连接 —— 跨引擎的对比不是「差异」而是「翻译」，写出来的脚本只能是猜的，所以直接不让选；比较的粒度是**表**：只有一边有的、两边都有但字段或索引不一样的、一模一样的，都列出来（列表上按 `only left` / `only right` / `differs` / `same` 分色，点开一条看它到底哪几个字段、哪个索引不同，两边各自的写法并排摆出来）；视图之类被跳过的对象**写进警告**而不是悄悄略过，否则「没有差异」会被读成「两个库一模一样」。**主键不算进索引差异**（它本来就是字段自己的标记，两边把同一个主键写成不同索引名只算一处），**两边都是自增的字段不比默认值**（PostgreSQL 的 `serial` 默认值指向的就是它自己那张表的序列，比文本只会把每个 serial 字段都报成不同）。`Generate script` 给出的就是这份差异本身 —— 表设计器那套词汇（字段、类型、标志、索引）本来就是一句 DDL 的词汇，所以**预览与执行由同一段渲染代码产出**，按「先建、再改、最后删」排（表得先存在才改得动，删掉的放最后，中途失败时创造的多过毁掉的）；唯一要用户拿主意的是 `dropExtra` —— 只有右边有的表默认**不动**，因为「让 B 变回 A」和「把 B 多出来的全删了」是两句不同的话，勾上会**重新规划一遍**而不是跳过其中几条。`Run script` 会**再规划一次**（真正跑的是此刻目录渲染出来的语句，不是一分钟前读的那份预览），逐条执行、失败就停下并报「第几条失败、前面几条已进」，而且**只有右边可以是只读的例外** —— 左边常常是拿来当参照的生产库，对它这里只读；两条边不能是同一个库（同一会话同一库会按构造得出「一模一样」，直接拒绝）。脚本跑完记**变更日志**（来源 `compare`，作用对象写的是那个库而不是某张表 —— 一次能改很多张）。
...
- **数据生成**：图标栏上的 **数据生成**、或表右键的 `Data generation…`，开一个「填表」窗口 —— 左边挑一张表（连接 → 库 → schema → 表，只列**表**：视图没有自己的 INSERT，集合没有列清单），右边就是那张表的字段表（Field / Type / Mock / Description）。**每行左侧有一个勾选框，勾上的列才会进 INSERT** —— 默认全勾，**自增列默认不勾**（那个键得由引擎发，自己编的值一轮写完就撞上了），表头那个框是全选 / 全不选。**空 mock 与不勾是两件事**：不勾 = 这一列不参与（描述列会写明），勾了却没写 mock 会被拦下来并点名是哪个字段，而不是拿空字符串冒充一个值 —— 勾中的行带品牌色底纹、未勾的行变淡（但 mock 仍可读可改）。**一个连接一个窗口**：表里写好的 mock 在同一个连接里换表不丢，从别的连接选一张表会切到**那个连接自己的窗口**（每个窗口只写它自己连上的那个库）。**跑完必须留下一句话**：进度条旁边一直走着**计时**，跑完的提示把**进去多少行、跳过多少行、用了多久**一起写出来（`Inserted 1,000 row(s) in 12.4 s — 3 row(s) skipped`），中途按停、被引擎挡下、整轮出错各是各的句子 —— 写进去的行撤不回来，剩下的就只有这句话。工具条上的 **Skip bad rows** 决定引擎拒绝一行时怎么办：**不勾（默认）就停在那一行**，提示里点名是第几行、已经进去多少行；勾上就**把拒绝的行数出来、跳过、接着往下写**，提示里给出跳过的行数与**第一句引擎原话**（一个老是在造表不肯收的值的 mock，是该被人看见并修掉的，所以默认是停下来）。
  - **Mock 是模板，不是固定值**：写的是 mock.js 语法 —— 字面文本里夹 `@` 占位符，`user_@natural(1, 999)@tld` 也是一个值。窗口里的取色器把分组摆在**左边**（Person / Web / Basic / Time / Character / Number，`@cname`、`@id` 带校验位、`@bankcard` 走 Luhn、`@address` 是省市区 + 街道 + 门牌号、`@guid`、`@now(day)`… 一共 40 多个；最后那一组 **Custom** 是用户自己攒的），读法跟**设置页**一模一样：左边一列组名、右边一条滚动盒子里连着摆下所有组，**往下滚就是下一组**（组名上的高亮跟着滚动走，点组名也真的把盒子滚过去），另外**滚轮落在组名上就换一组**（滚轮每格走一组，一格里的连续事件只算一次，所以手抖不会连跳好几组；已经到头就不再吃掉这个事件，交给弹窗自己滚），每个占位符是**一块平铺的卡片**（**四个一行**：卡片窄了名字与示例会折行，而不是被切掉）而不是一行 —— 卡片上除了名字，直接用同一个引擎渲染一行**示例值**（同一种模板给同一种样本，重开窗口也不会乱跳）：光看名字分不出两个相似的占位符，看样本就分得出来。也可以直接手打；**认不出的 `@name` 是错误而不是原样输出**（mock.js 会把 `@nope` 留下来，于是打错一个字就得到一列同样的错字符串），单元格当场标红、Generate 拒绝执行并点名是哪个字段，要写一个真正的 `@` 就用 `@@`。模板引擎在 `frontend/src/lib/mock/`（词表、校验位、参数解析都是本地实现，一处纯函数、可给种子复现）。
  ...
  - **写下去之前先问一次**：确认框说明要写多少行、发哪几列、进的是哪张表，并**摆出这批行要发的那条 `INSERT`** —— 后端渲染的形态：表与列都是标识符、值以绑定参数出现（一批 500 行的字面量既不值得读、也长得离谱），并写明**没有 undo**（撤不回来，要删就按普通行删）。跑完记**变更日志**（来源那一栏是 `datagen`：一批 200 行就是**一条**记录，带上引擎报的**落库行数** —— 那样表里凭空多出来的行也有来历，见「变更日志」）。**同意之后弹窗就关掉了**：跑的这段时间只由这个窗口交代 —— 进度条、Stop、最后那句话都在它原来的位置上，不会有一个确认框挂在进度条上面转圈。**一轮最多写多少行是设置项**：默认 100 万，在 **设置 → Data generation → Rows per run** 里改（上限 1 亿、下限 100，存成数据目录里的 `datagen.json`），工具条上 Rows 框的 `max` 与旁边那句 `max …` 就是它 —— 数字从后端读回来（`DataGenSettings`），所以设置页改完当场生效；多打一位的数字（比如把上限 100 万写成了 500 万）在一轮里只会按上限走，离开输入框时框里的数也落回上限，而不是写进去一半才发现。生成在前端、分批过桥：每批 200 行（后端上限 500），一个批次一条多行 `INSERT`，进度条在两批之间走、**Stop 也在两批之间生效**，所以停下来时报的数就是真的写进去的数。后端 `InsertRows` 只把标识符（表名、列名）按方言引号拼进语句，**每一个值都是绑定参数**，前端生成的任何文本都不可能变成 SQL；JSON 过桥后整数都成了 `float64`，落库前把**整数值折回 int64**（PostgreSQL 会把 `float64` 绑到 `int4` 上直接拒收，MySQL 则默默四舍五入），小数原样交给列去判断。引擎拒掉一批时，后端**逐行重放**找出是哪一行并把引擎自己的话带回来，窗口如实报「第 N 行被拒，前面 m 行已进表」，不会把一半说成全部。
- **变更日志**：图标栏上的 **变更日志** 开一页「这个程序对数据库做过什么」—— 左边按**最新在前**列出每条记录（时间、语句的首关键字、作用在哪个对象上、语句摘要），右边是选中那条的全部内容：**日志时间 / 连接信息（连接名 + 引擎 + 用户与地址）/ 数据库（`库 · schema`）/ 表名称 / 从哪个窗口执行的 / 影响行数 / 执行语句**（按引擎高亮），失败的那条还带一段红色说明（列表上带行数的那几条也会顺手印出来）。顶部有计数（`the newest 500 of 1,248 statements`）、过滤框与重新读取。它读的是**数据目录里的文件**而不是任何一个服务端，所以**没连库也能打开** —— 而变更通常正是连接已经关掉之后才要查的东西。记录的是**这个程序被要求执行过的写语句**：结构页保存（一条一条执行，所以每条都有自己的结果）、DDL 编辑器 / 查询窗口 / 新建库里跑过的脚本（按语句切开，只留 `ddl` / `dml`，读语句与认不出的语句不记）。**网格里的改与删现在也记**（来源那一栏是 `grid`）：行详情会把引擎渲染出的那条语句摆出来给人看过再执行，内联改单元格与删行走的是同一段渲染 —— 日志里那行就是真正发出去的那个字符串，不是为日志另拼一次，发出去又失败的语句也照记（日志回答的是「这个数据库被要求做过什么」）。**树里直接清空或删除一张表也记**（来源那一栏是 `explorer`，作用对象就是那张表）：它和建表、复制走同一段逐条执行的代码。**复制表也记**（来源那一栏是 `copy`，作用对象写的是新建的那张副本）：它跟建表走同一段逐条执行的代码，所以哪一条失败、前面几条已经进去几条都照实写下来（数据那一条用 `INSERT … SELECT`，日志里存的就是这么一句，不展开成具体行）。**数据生成也记**（来源那一栏是 `datagen`）：一批 200 行是**一条**多行 `INSERT`，所以**一批一条记录**——记的是这条语句的形态（表、列，值以绑定参数出现）和引擎报的**落库行数**；一行一条地记只会把日志淹掉，一批一条地记才既看得懂、又答得上「这批数据是什么时候进去的」。**跑一份 SQL 文件也记**（来源那一栏是 `sqlfile`）：文件是**一条语句一次调用**跑的，所以每条写语句都是一条自己的记录，也就各自带着引擎报的**受影响行数** —— 这跟「一段脚本记一条」刚好是两种记法，区别在于语句是一条一条发出去的还是整段发出去的。每条记录还带着**这个语句改了多少行**（引擎自己数的 `AffectedRows`）：「一条 `UPDATE` 跑过」和「一条 `UPDATE` 改了 4120 行」是两件不同的消息；结构语句没有行数、失败的语句（什么都没改）也不带，一次跑了好几句写语句的脚本同样不带 —— 引擎对整段只回一个结果，把那个数字安在其中一句头上比不写更坏。日志**一天一个文件**（`<数据目录>/log/<yyyymmdd>.log`）、**只轮转、从不丢弃**：写满一个文件（**默认 2000 条，可在 设置 → 数据目录 里改**）就整份改名成 `log/20260214-1.log` 留档、同一天另起一个新的，所以上面那个下拉框里能选出**历史日志**来读（每个文件都标着里面有多少条、多大），右边读的就是选中的那一份；更早的版本把日志放在数据目录根上（`changelog.jsonl` 与同形状的留档），那些照样列出来、照样读得出来。
...
- **运行 SQL 文件**：库节点右键的 `Run SQL file…` 让**后端读盘上的那份 `.sql`**、一条语句一次调用地跑下去 ——文件本身不过桥（过桥的只是一个路径），所以比内存大的 dump 也能跑，**进度条按字节走**（读了多少就是多少），Stop 取消的是正在跑的那条语句连同后面全部。跑之前先**读一遍文件**告诉你里面有什么：几条语句、头 50 条各是什么（可能丢结构或数据的标红）、只读连接会拒掉几条、认不出的语句与 `USE` 都进警告（`USE` 是**跟着走**：后面的语句改发到那个库，这一句本身不发给引擎）；跑完给的是总结 —— 跑了几条、失败几条、一共改了几行、用了多久，以及**头几条失败在文件里是第几条、引擎的原话是什么**（照着编号就能回去改那一条）。几条直说的话：**文件没有事务包着**，中途停下或遇错停下，已经跑过的那几条都留在库里（`Stop at the first failure` 默认开着，要一路跑到尾就关掉它）；**每一条写语句进变更日志**（来源 `sqlfile`），而且**各自带着自己那一行数** ——脚本一次调用时引擎只回一个结果，那个数字只能记在整段上，或者干脆不记。只有能跑 SQL 的引擎有这个入口（MongoDB 的语句是 shell 调用），也不摆在 schema 节点上：文件自己点名对象，跑的是整个库。
- **外观**：Navicat 式窗口骨架（icon-over-label 命令条 + 最左那条五页图标栏 + 连接树 + 标签页工作区 + 状态栏）、明暗主题、品牌绿 `#36ab60`、可拖拽分栏、紧凑的表格与状态栏 —— 表格的表头**固定不动**：数据网格、对象列表、设计器的字段表都一样，纵向滚动时表头留在容器顶上，横向滚动也带不走它（钉住的列仍旧钉在原处）。主题有三档：`Light` / `Dark` / `System`，在设置页的 **Appearance** 里选，Windows 上托盘菜单的 **`Display theme`** 是同一个偏好的另一个入口（见「托盘只有 Windows 有」）（跟随系统时它会写成 `system (dark)`，把当前系统给的那一档一起说出来）；选**跟随系统**时窗口真的跟着操作系统走 —— 操作系统在运行期间切深浅色，界面当场就变，不用重启也不用再点一次。状态栏右下角那一格是**浅色 / 深色两档的开关**，写的永远是**屏幕上正在用的那一档**（不是存下来的偏好：`System` 不是一个能被点亮的状态，而是「为什么现在是这一档」），点一下平移到另一档；正在跟随系统时，提示里会说明这一点并告诉你会固定成哪一档（`Light` ⇄ `Dark`），所以「跟随系统」不会变成一个够不着的偏好 —— 托盘与设置页照旧管它。紧挨着它的是**界面语言那一格**：上面只写**当前语言**（`EN` / `简` / `繁`，用它自己的语言写），点一下按 `EN → 简 → 繁 → EN` 往下轮一档，完整名字和「点了会到哪一档」都在悬停提示里 —— 这两格都是**状态栏里的快捷入口**，摆的是现在这一档，而不是把三个选项都亮着（26px 的一条 readout，不该变成一行要读的按钮）。换语言还有设置页的 `Language` 与 Windows 托盘的 `Interface language`，三个入口改的是同一个偏好。命令条上目前只有 **Connection**、**Open**、**Close**、**New Query**、**Refresh**、**Table** 与 **View** 是活的，其余按钮保持原来的位置但禁用并在提示里说明 —— 摆着的空位比消失的按钮更好认。切**页面**的命令不在命令条上，而在最左边那条图标栏里（见「页面栏与五个页面」）：命令条只管**作用在当前连接上的事**。
- **设置**：最左边图标栏上的 **设置** 开一整页（不再是弹窗 —— 它是「去一趟再回来」的地方，不是盖在手上的东西）—— 左栏是栏目、右边是这五个栏目共用的一条滚动区，于是栏目名**跟着滚动高亮**（滚到哪一段就是哪一段），点栏目也真的把内容滚过去 —— `Appearance` 选主题，`Language` 选界面语言（English / 简体中文 / 繁體中文，三档，**默认英文**，见「界面语言」；Windows 上托盘的 `Interface language` 是同一个偏好的另一个入口），`Code generation` 选代码窗口默认用哪种语言，`Mock placeholders` 管自己的占位符（见上），`Data folder` 看数据存在哪、里面有哪些文件（每个文件写的是干什么的、多大 —— 连接配置、查询收藏、树的排法、窗口状态、密码密钥、**变更日志**、**日志的轮转设置**，还有 `.query` / `.mock` / `log` 这三个文件夹 —— 后三者按「一个文件夹、里面几个文件」列出来（`log` 里就是这一天一份的 `<yyyymmdd>.log` 与 `log/<yyyymmdd>-N.log` 留档））、**顺手改日志一个文件写多少条**（到数就整份留档，见下）、从这里**打开目录**、**换一个目录**或**恢复默认**，`About` 就是原来那页项目信息（技术栈徽章、项目地址、开发者）。每一页都是**改了就生效**，所以没有「取消」可点，也没有哪个选择会因为走开而丢掉。换目录是真的**把数据搬过去**：连接配置、查询收藏、树的排法、窗口状态与密码密钥一起复制到新目录（逐个读回校验，对不上就不动原文件），然后才改指针、最后才删旧文件；没能删掉的（被占用、只读）会**如实列出来**，而不是回一句「已移动」。目标目录里已经有**非空的**本程序数据时会被拒绝并点名是哪个文件（同名但零字节的不算数据，照常覆盖），选到当前目录本身也会被拒绝；那些**不是本程序写的**文件留在原地并在结果里注明。目录就绪后不需要重启 —— 会话照旧连着，新的保存当场写进新目录。
...
- **托盘**（Windows）：关掉窗口是**收进托盘**，不是退出；托盘图标左键单击等于把窗口叫回来，右键的菜单从上到下是 `Show window` / **`Display theme`**（`Light` / `Dark` / `System`）/ **`Interface language`**（`English` / `简体中文` / `繁體中文`）/ `Project page` / `Report an issue` / 版本号（灰色，只是给你看的）/ `Quit`，两个设置里**当前那一档打着勾**、整个菜单也**按当前语言写**。这两组是设置而不是动作：窗口收进托盘之后，菜单是**唯一够得着的地方** —— 语言选错了读不懂、深色看着刺眼，都得靠它回到窗口（或者不必回到窗口）。挑中哪一档走的是**和设置页同一条路**（见「托盘只有 Windows 有」）。`Quit` 走的是正常退出（连接池照常关），而**图标没能建出来时关窗照旧直接退出** —— 一个没能出现的托盘不该留下一个看不见的进程。其它平台没有通知区域，行为保持原样（关窗即退出）。
...
├── tray.go                   # 托盘菜单的内容与动作（文字 / 顺序 / 勾选 / 语言只有这一处）
├── tray_windows.go           # 纯 Win32 的托盘：隐藏窗口 + 自己的消息循环 + Shell_NotifyIcon + 画菜单
...
│   ├── config/changelog.go   # 变更日志：<数据目录>/log/<yyyymmdd>.log（一天一个文件）+ 满 2000 条整份留档成 log/<yyyymmdd>-N.log + changelog.json 记轮转阈值（老版本的 changelog.jsonl 仍读得到）
│   ├── config/datagen.go     # 数据生成的行数上限：<数据目录>/datagen.json（默认 100 万，100 – 1 亿）
...
│   │   ├── sqlutil/          # 标识符引用、WHERE / ORDER BY 构造（纯字符串+参数位）、脚本干跑、流式切语句（split.go）
│   │   ├── sqlbase/          # 通用 database/sql 实现：连接池、分页、脚本执行、DDL（design.go 的规划器 + copy.go 的复制表 + tableops.go 的清空 / 删除 + diff.go 的两张结构对比）、行变更（含 literal.go 的语句预览与导出共用的字面量）、导出用的整表流式读（stream.go）、运行情况外壳
...
│   │   └── all/              # 汇总导入，保证 init 注册
│   └── service/manager.go    # 会话管理、超时、只读校验、审计入口（compare.go：两个库的比较与同步脚本；export.go：整库 / 整表的导出，边读边写；sqlfile.go：跑一份 .sql 文件，一条语句一次调用）
└── frontend/
    └── src/
        ...
        │   ├── ActivityBar.tsx # 最左边的图标栏：连接 / 数据生成 / 比较 / 变更日志 / 设置五页唯一的门
        │   ├── SettingsPane.tsx # 设置页（不再是弹窗）：外观 / 界面语言 / 代码生成 / Mock 占位符 / 数据生成 / 数据目录（含日志轮转阈值）/ 关于
        │   ├── DataGenWorkspace.tsx # 数据生成页：一个连接一个窗口的标签栏，窗口本体在 DataGenPane.tsx
        │   ├── MockPickerModal.tsx # 占位符取色器（组名在左、卡片在右，跟设置页同一套读法：滚一下换一组，卡片带示例值）
        │   ├── MockPlaceholderSettings.tsx # 自定义占位符：列表 + 编辑器（带种子的调试面板）
        │   ├── ChangeLogPane.tsx # 变更日志：顶部选文件（当前 / 留档）、左边最新的记录、右边那条的全部内容（连接 / 库 / 表 / 语句）
        │   ├── ComparePane.tsx # 数据库比较：两边各挑连接 / 库 / schema、列出逐表差异、生成并执行同步脚本（先建、再改、最后删）
        ...
        │   ├── CopyTableModal.tsx # 复制表：名字（默认 `<表名>_copy`）+ 「只要结构 / 结构和数据」两档，边打边看后端渲染的语句与警告
        │   ├── ExportModal.tsx # 数据库导出：三档（只要结构 / 结构和数据 / 只要数据）+ 选表（数据档单选并可选字段）+ CSV / JSON / JSONL / SQL + 进度与 Stop，读写都在后端
        │   ├── RunSqlFileModal.tsx # 运行 SQL 文件：挑一份 .sql，读过之后摆出里面有什么（语句、会丢数据的、会被只读拒的），再一条一条跑，进度 / Stop / 失败清单都在这里
        ...
        ├── lib/              # tree key 编解码、格式化、导出、代码生成（codegen/，见下）、驱动能力（capabilities.ts）、项目信息（about.ts）、主题（theme.ts）、品牌素材（assets.ts，@asserts 别名）、左栏 + 滚动区（sectionScroll.ts）
        │   ├── codegen/      # 代码生成：types.ts（类型归类与渲染契约）/ languages.ts（每种语言的数据）/ lexis.ts（每种语言怎么读）/ highlight.ts（生成代码的高亮）/ index.ts（入口）
        │   ├── i18n/         # 界面文案：区域消息表（messages/）、t / tn / tr、antd locale 与语言常量（见「界面语言」）
        │   ├── tray.ts       # 托盘菜单里那两档设置的回程：听 Go 发来的事件，走 store 的 setTheme / setUiLanguage（见「托盘只有 Windows 有」）
...
- 批量 INSERT 尽量发成**一条多行语句**（一次往返而不是一行一次），引擎不接受时才**逐行重放** —— 多行语句只有「整条成不成」，重放是唯一能说清「是哪一行」的办法。重放最坏会在 INSERT 不是逐条原子的引擎（MySQL 的非事务表）上把那条语句已经写进去的行再写一遍，那会被报成「这一行失败」并带上引擎原话：界面上的计数始终是**真落进去的行数**，这是半张表面前唯一要紧的事。
- 重放到被拒绝的行时是**停下还是跳过去**由请求里的 `SkipErrors` 决定（就是数据生成窗口那个勾选框），服务层只负责把它传下去：默认停，因为一个老是在造表不收的值的 mock 该被看见。
...
窗口最左边是一条**图标栏**：**连接**（工作区 —— 连接树与连接开出来的各个窗口）、**数据生成**、**比较**、**变更日志**、
**设置**。它不是装饰品，而是这五页唯一的门：连接树这一格窗格是可以收起来的，收起来之后就再没有地方把它叫回来，
所以得有一条**自己不能被藏起来**的东西来给它们命名，而这条栏也不可能被任何一页带着消失（五页都是它的内容）。

切页面**不是重建页面，而是先挂载、之后一直挂着（只是隐藏）**：数据生成窗口里写了一半的 mock、变更日志里
正读着的那一条、比较页上正摆着的两个库，都不该因为走开一眼就没掉 —— 跟标签页把非当前页留在 DOM 里是一样的理由。所以主区域不是
「一个窗格换内容」，而是五层叠在一起、只显示其中一层（`AppShell` 的 `PageLayer`）；每次切回来时各页按需重新读一遍
数据目录或库（它们的 `active` 就是 `page === …`），因为隐藏期间数据可能被别的窗口改过 —— 比较页是例外，
它是按按钮才读的：一次比较要把两边的每张表都读一遍，没有理由替用户重跑。

由此**数据生成 / 比较 / 变更日志 / 设置不再出现在工作区的标签栏里**：它们不是「某个连接上的一个窗口」（数据生成是
**一个连接一个窗口**，按连接而不是按表分组；比较是关于两个连接的，两边都不属于；变更日志与设置根本不属于任何连接），所以它们各自成页，
从标签栏里被滤掉（`Workspace`）。命令条上也随之**没有**这几枚按钮 —— 切页面的命令就待在页面旁边。
...
| 一个库 / schema 节点 | ER 图 / New DDL script…（关系型才有）、Run SQL file…（关系型才有，只摆在库节点上 —— 文件自己点名对象，跑的是整个库）、Export structure · Export structure and data · Export data（关系型才有）、New query…（配了连接才有）、Reload objects |
| 一个对象文件夹（`Tables` / `Views` …） | Open object list / New table…（`Tables` 才有）/ New DDL script…（关系型才有）/ Reload objects |
| 一个对象（表 / 视图…） | Open data / Design table（给不出设计器的引擎是 Open fields）/ Edit DDL…（关系型才有）/ Generate code…（有字段的对象才有）/ Data generation…（表 + 能 INSERT 的引擎才有）/ **Duplicate table → Structure only · Structure and data**（表 + 能建表的引擎才有）/ **Truncate table** / **Drop table**（表 + 能建表的引擎才有）/ Copy name |
...
读语句与认不出的语句不记）、`Manager.UpdateRow` / `Manager.DeleteRow`（网格的行编辑与删行：语句由驱动渲染成文本，**同一份文本**既是预览也是日志那行，所以日志里写的就是发出去的那条），以及 `Manager.InsertRows`（数据生成窗口那批凭空造出来的行：一批 200 行走的是一条多行 `INSERT`，所以**一批一条记录**，记的是这条语句的形态与引擎报的落库行数）。窗口只负责把「这段脚本是写在哪个对象上的」告诉服务层（`ExecRequest` 多了
...
**每条记录都带引擎报的受影响行数**（`Rows`）：「一条 `UPDATE` 跑过」和「一条 `UPDATE` 改了 4120 行」是两件
不同的消息。数字取的是引擎自己数的（`RowsAffected`），数不出来的地方就不写：结构语句没有行数可言，失败的语句
在 `logStatement` 里被清零（`runErr != nil` 就是「什么都没改」，前面某一步的计数不能拿来充数），而一次跑了好几句
写语句的脚本也不带 —— 引擎对整段只回一个结果（`res.AffectedRows`），只有这段脚本里**恰好一句**写语句时才认领得了，
把那个数字安在其中一句头上比不写更坏。数据生成那批的行数是**真落进表的行数**：引擎拒掉半批而窗口勾了跳过时，
一批仍然是一条记录，数字是实际进去的那些（「第 N 行被拒」由窗口当场说，不由日志事后补）。
...
**一天一个文件**：`<数据目录>/log/<yyyymmdd>.log`，一行一条 JSON。按天分而不是「一个文件写到满再换」，是因为
读日志的人问的是「14 号都跑了什么」，那是一份文件，而不是一条卷里要去找的一段。一行一条 JSON 而不是跟其他文件
一样的「一个文件一份 JSON 数组」，理由是这个文件的使用方式跟它们不同：它被追加的次数远多于被读的次数，
方向永远是往末尾加、从开头读，而且它是这里唯一一个不需要用户同意就会长的文件。于是一行读不出来的 JSON 只丢那一条
（崩溃留下的半行、手敲坏的一行），而不是整个日志打不开；读取走 `readChangeLogTail`（只看最后 8 MiB，且丢掉被截断的
第一行），所以别人往里塞过东西也不会把整个文件拉进内存。

**写满了是留档，不是丢弃**（`rotateChangeLogLocked`）。一天的文件写到用户设的条数（默认 **2000**，存在
`<数据目录>/changelog.json` 的 `maxEntries` 里，设置页的 `Data folder` 页改它）就**整份改名**成
`log/<yyyymmdd>-N.log` —— 这一天，加上这天第几次轮转 —— 然后同一天再起一个新文件接着写。用改名而不是
重写/截断，是因为改名是**一步原子操作**：不复制、不重写，正好在读它的人要么拿到完完整整的旧文件、要么拿到
完完整整的新文件；于是「删掉最旧的几条」这件事根本不需要存在，用户见过的每一条都还在。一天里轮转第二次就顺着
取 `-2`，两种占着的名字都会避开（`log/` 里已有的，以及数据目录里老版本留下的同名文件 —— 读的时候是 `log/`
里的优先，把那个名字发出去就会把老那份藏起来）。阈值那个文件是**单独一份**而不是塞进 `state.json`：轮转发生在
后端的一次 append 里，那里的后端得能读它，而 `state.json` 是前端整份替换的、后端不该去读别人拥有的文件。
设回默认值就把这份文件删掉 —— 没选过和选成默认不是两种状态。`AppendChangeLog` 是**追加**（不是整份重写）：
该重写的理由（截断）已经随轮转一起消失了，剩下的一次小写反而更不容易留下半行。一天里一条记录落进哪份文件，
由**这条记录自己的时间**（`entry.At`）决定，不是写的时候的钟 —— 界面上语句旁边显示的就是那个时间，午夜差一分钟的
一条不该落到第二天那份里，而调用方不填时间时就只能是「现在」。

日志文件夹是**文件夹，不是围墙**：更早的版本把日志放在数据目录根上（`changelog.jsonl`，留档也叫
`<yyyymmdd>-N.log`），那些照样列出来、照样读得出来，同一个名字两处都有时以 `log/` 里的为准 —— 那才是程序
正在写的那份；排序上它们排最后，名字里没有日子的那份是这里最老的东西。

日志因此是**一架子**文件，读日志就是先选哪一份：`Store.ChangeLog` 会在一次调用里同时给出选中的那份（最新在前）
与全部文件（今天那份在前、其余按新旧排，每份带条数、大小、留档时间），窗口顶部的下拉框就是这个架子 ——
分成两次调用的话，两次之间刚好发生一次轮转，列表与内容就会互相矛盾。留档文件名是**轮转的那一刻**定下来的，
所以它不可能事先写进 `dataFiles`：凡是需要知道数据目录里有什么的地方（设置页的清单、搬家、拒绝目标目录、
以及「还剩下什么不是本程序的」）都走同一个 `dataFileNames`，而它把数据目录扫一遍取出老版本留下的那些
`isChangeLogArchive` 认得的名字 —— 一份老留档会被列出来、被搬走、且不会被当成别人落下的文件。名字是读日志的
**唯一一道门**（`ChangeLogName` 白名单）：只认 `changelog.jsonl`、`<yyyymmdd>.log` 与 `<yyyymmdd>-N.log`
这三种形状，其余（路径、`connections.json`、`changelog.json`）一律拒绝，不能被「洗干净」成恰好安全的东西。
...
「导出」在这个程序里是**两件事**，因为它们的数据路径正相反：

| | 谁序列化 | 数据从哪来 | 入口 |
| --- | --- | --- | --- |
| 查询结果导出（`frontend/src/lib/export.ts`） | 前端 | 网格里已经有的那几行 | 网格工具条 / 行详情 |
| 数据库导出（`internal/service/export.go`） | 后端 | 驱动**一次**把整张表读出来 | 库 / schema 节点右键 |

后者不能走前者那条路：一整个库的行不可能先过一遍桥，而「一页一页地取」在本地看起来是分页、在服务端是
**每次都要重新跳过已经发过的那些行**（一个线性的问题被问成了平方的）。所以数据库导出的设计只有一句话：
**打开目标文件，按表读一行、写一行、忘掉一行**。内存里永远只有一行的宽度，进度、行数、字节数是这条流水线
顺手数出来的，Stop 就是取消那个 `context` —— 也正因为如此，目标路径必须在**开始之前**就问清楚
（`PickSavePath` 是从 `SaveTextFile` 里拆出来的前半段），写了一半再问「存哪儿」是没有意义的。

读行是一件可选能力：`drivers.RowStreamer`（`StreamRows(ctx, StreamRequest, each func([]any) error) error`），
由 `sqlbase` 一处实现、所有 SQL 引擎共用，MongoDB 不实现 —— 于是「这个引擎说得出它的表结构」和
「这个引擎的行能流式读」是两件独立的事，后者缺了只会让**带数据的那几档**不可用，纯结构导出照样能跑
（`unsupported` 只在真的要读行时才报）。`StreamRequest` 里**没有超时字段**：调用者的 context 是唯一的期限 ——
一次扫描跑上一分钟不是「慢查询」而是一张大表，而谁在等只有界面知道（它手里有 Stop），所以 `sqlbase`
那套默认 60 秒的查询预算**故意没有**套在这里。

进度走 Wails 事件 `export:progress`（每 100ms 一次，开跑与每张表写完之后各强制发一次），`done` / `total`
数的是**表**：一张表有多少行只有读到底才知道，为了画一条百分比先把每张表扫两遍是本末倒置。
`Manager.ExportDatabase(req, progress func(...))` 里的 progress 是**一个 Go 参数而不是 DTO 字段** ——
`models` 里因此没有一个函数字段，而这个回调**跑在调用者自己的 goroutine 上**（Wails 给每个绑定方法一条），
所以「最后一个进度」不可能落在总结之后。

几条明确的取舍：

- **表能读出来几张就写几张**。读不了的那张表进 `Warnings`（带引擎的原话）、run 继续；只有**不属于某一张表**
  的失败（没有会话、没给目标、磁盘写不动）才是错误并结束这一趟。半份 dump 留在原地 —— 那是用户要停的那半份。
- **一次失败是「请求错了」还是「这张表读不出来」要分清**。`CodeInvalidConfig`（勾了一个不存在的字段）在
  **文件还没建出来之前**就对目录校验一遍，run 里还有一道 `fatal` 兜底 —— 一份少了一列的 dump 要到恢复时
  才会被发现，这是逐表警告这条规则唯一不能吞的失败。
- **结构与数据都从引擎读，不从本程序的渲染器来**。`Conn.Structure` 是结构页用的那一个调用，所以 MySQL 给的是
  `SHOW CREATE TABLE` 的原话，键、选项、存储子句都在；引擎答不出自己的 DDL 时才有 `RenderCreateTable` 兜底。
  SQLite 的索引**不在表的 `CREATE` 里**（它自己占 `sqlite_master` 的一行），导出照实写引擎的话，
  不去替它把 `CREATE INDEX` 折进表定义 —— 那会改掉结构页的含义。
- **一行一条 `INSERT`，列清单每行都写全**。批量 `VALUES` 对短行更省，但它必须找个地方切开
  （MySQL 的 `max_allowed_packet`、SQLite 的语句长度），切开之后一次失败丢一批而不是一行，而一行一条的文件
  是能用眼睛看、也能从中间接着跑的。值用方言渲染的**字面量**（`sqlbase.Literal`，与结构预览同一段渲染代码），
  所以才认得出 MySQL 里反斜杠是转义字符、PostgreSQL 里它不是 —— `scan.go` 那个方言无关的 `QuoteLiteral`
  在这里是错的工具。
- **JSON 里的数字必须是数字**。驱动交回来的 `int64` / `float64` 由 `jsonNumber` 显式列出来渲染
  （这份名单与 `sqlbase` 的字面量渲染器是同一份），不是「认不出来就按文本引号包上」—— 一份计数带引号的 dump，
  每个读它的人都要先洗一遍。`NaN` / `±Inf` 例外：JSON 没有它们的写法，只能落成带引号的字符串。
- **JSON 数组即使中途停下也要合上 `]`**：截断的数组不是 JSON，已经落进去的行还应该读得出来。对象是**手工拼**的
  而不是 `Marshal(map)`：map 没有顺序，每行字段乱跳的文件没法 diff。
- **CSV 的规则与前端那一份逐字相同**（RFC 4180，含逗号 / 引号 / 换行的单元格加引号，行尾 CRLF），但两处代码
  没有共享：它们跑在桥的两边（网格手里有行、这边看不见行），复制四十行比把任何一边搬过去便宜。
- **视图不导出**，被跳过的个数在窗口里写出来 —— 视图的定义是一条查询，按表的样子写出去只会得到一份跑不起来的文件。

### 运行 SQL 文件：后端读盘，一条语句一次调用

库节点右键的 `Run SQL file…` 把磁盘上的一份 `.sql` 跑在一个库上，**文件不过桥**：过桥的只是一个路径。
一份 dump 常常比内存大、比这条桥愿意搬的东西大，而「谁来读」正好决定了另外两件事成不成立 —— 进度是真的
（读了多少字节就报多少），Stop 也是真的（取消那个 `context`，正在跑的那条语句连同后面整份文件一起停）。

它跟 DDL 编辑器不是一回事，两边都在跑脚本，但账算得不一样：

| | 语句从哪来 | 怎么发出去 | 变更日志 |
| --- | --- | --- | --- |
| DDL 编辑器 / 查询窗口 | 用户当场敲的，整段过桥 | 一次调用跑整段 | 整段记一条（脚本级的 `AffectedRows` 不记在各条头上） |
| 运行 SQL 文件（`internal/service/sqlfile.go`） | 磁盘上的文件，逐个读 | 一条语句一次 `executeScript` | 一条写语句一条，各自带自己的行数 |

**一条语句一次调用**是这里的全部技巧：`Manager.executeScript(ctx, req, source)` 就是查询窗口用的那一段
（只读拦截、按语句的超时、写语句进变更日志都在里面），文件只是每次只交给它一条 —— 于是日志里那条语句就是
真正发出去的那条，而「每一句写语句都带着自己那一行数」也自动成立（一次调用里只有一条写语句，
`logScript` 那条「一次跑了好几句就不记行数」的规则不会被触发）。`source` 参数就是日志里「从哪个窗口执行的」
那一栏（`sqlfile` 与 `script` 分开）。

几条明确的取舍：

- **没有事务包着**。`Conn` 上根本没有事务句柄，DDL 也不是每个引擎都回滚得了，所以文件是**一条一条提交**的：
  中途停下、或者遇错停下，已经跑过的那几条都留在库里，`Failed` / `StoppedOnError` 说的是「停在哪里」，
  不是「什么都没发生」。界面上直接写出来（「中途停下不会回滚已经执行的语句」），而不是让人自己发现。
- **`USE` 是跟着走，不是发出去**。池化连接上的 `USE x` 落到哪条连接上是不确定的，所以从语句里读出来的库名
  用来重定向**后面**的语句（每条语句都带着自己的库发出去），这一句本身既不发给引擎、也不进日志 ——
  它不是对数据库做的事，而是文件里的一句交代。名字读不出来（`USE a b`）报的是**这一条**的错误，
  反引号 / 引号包着的名字带空格是认的（`USE \`my db\``）。SQLite 的 DSN 用文件路径、库参数本来就被忽略，
  PostgreSQL 的文件里也没有 `USE`，所以这条重定向对这两家是空操作。
- **切语句在后端、且是流式的**（`internal/drivers/sqlutil/split.go`）：扫描器一个字符一个字符读，所以比内存大的
  文件也能跑，而「读到哪里」正好就是进度。它认 `DELIMITER` 指令（mysqldump 的例程就靠它）、认 PostgreSQL 的
  `$tag$…$tag$` 整段字符串（函数的函数体活在这对美元号里）；判断不分引擎方言 —— 一份文件里 `a$b$c` 这种
  标识符会被当成美元引用（MySQL 里少见，在 PostgreSQL 里它本来就该是那个意思）。
- **进度按字节算**。文件没读完之前谁也不知道里面有几句，所以进度条是 `已读 / 文件大小`（`stat` 来的），
  旁边数的是**跑过几条、失败几条、改了几行**。事件 `sqlfile:progress` 每 100ms 一次，开跑时强制发一次。
- **被 Stop 打断的那条语句不算失败**。引擎回错误时如果 `ctx.Err()` 已经非空，说明它没来得及拒绝这条语句，
  记录的是「停下了」，而不是凭空造一条失败出来。
- **失败是结果的一部分，不是 Go 错误**。一份跑到一半的文件回的是总结（跑了几条、失败几条、头 20 条失败的
  语句编号与原话），窗口照着画；只有「会话没了 / 文件打不开 / 给的路径是个文件夹」这类**请求本身的问题**
  才抛异常 —— 一次失败要能区分是「这趟请求错了」还是「文件里有一条跑不过去」。
- **只读会话不是拒绝运行，是拒绝写**：读数里指出「有几条会被拒」，跑的时候那几条各自报错，读语句照跑，
  跟查询窗口守的是同一条规矩。
- **只有句子是 SQL 的引擎才有这个入口**（`DriverInfo.Relational`）：MongoDB 的语句是 shell 调用，
  一文件 SQL 对它没有意义 —— 这仍然是能力判断，不是引擎名单。

### 写操作：先看语句，再看改了多少行

**这个程序自己生成的写语句，一条都不在没打过招呼的情况下发出去。** 表设计器保存、新建表、清空表、删除表、
复制表、网格里的改行与删行、数据生成那批批量插入、比较页的同步脚本、新建数据库 —— 点下去之前都有一个确认框，
框里是**后端渲染出来的那条语句**（`SqlCode` 高亮，就是「结构查看器」用的那套读法），下面才是给人看的说明
（影响几行、哪张表、有没有 undo、这个引擎做不到的地方）。预览与执行走的是**同一段渲染代码**：确认框里那段文本
就是发出去的那段，不是为确认另拼一次；真正执行时后端还会**再规划一遍**（`ApplyDesign` 只收设计不收 SQL），
所以中间目录变了也不会拿一分钟前那份预览去跑。

删多行时框里是**一行一条 `DELETE`、按将要执行的顺序**排出来的整份清单 —— 「删了几行」这句话在这里没有意义，
用户要看的正是那几条语句本身。

**用户自己敲在查询窗口里的 SQL 不再多问一次**：那是用户写的、就在眼前，他按下的就是执行键；再问一遍只是把
「执行」变成两次点击。它照样记进变更日志（日志回答的是「这个数据库被要求做过什么」，与是谁写的无关）。

确认框答「不」是**不做**，不是失败（`useStatementConfirm` 返回 `false` 而不是抛异常）：网格里那个单元格随即回到
服务端的值 —— 放弃一次改动是决定，不是错误。

### 设置：主题、界面语言、代码语言、Mock 占位符、数据生成与数据目录

页面的形状是**左栏 + 一条滚动区**，而不是「卸掉上一段、挂上下一段」：六个栏目（外观 / 代码生成 / Mock 占位符 /
数据生成 / 数据目录 / 关于）同时在场，栏目名跟着滚动位置高亮，点一下栏目就真的把内容滚到那一段去。所以「现在是第几段」
只有一个来源 —— 滚动位置 —— 点击只是去改它，而不是另存一份会跟滚动打架的状态。两处细节：点击自己引起的
滚动期间不读滚动位置（否则高亮会在动画途中把路过的几段都报一遍），以及最后一段可能短到永远滚不到顶部，
所以滚到底就当到底。也正因为六段同时在场，离开这一页再回来时它们都还在原处 —— 包括 Mock 占位符里正在写
的那张表单。这套形状不只是设置页的：占位符取色器（`components/MockPickerModal.tsx`）共用同一段逻辑
（`lib/sectionScroll.ts`），所以「哪一段算在眼前」的规矩只有一份 —— 同一个滚动位置在两个地方不会有两种读法；
取色器只多一条自己的：滚轮落在组名上就换一组。
...
设置里**不进 `state.json`** 的是两个数字：日志轮转阈值存在 `<数据目录>/changelog.json`，数据生成的行数上限存在
`<数据目录>/datagen.json`，各自跟它管的那件事待在一起。理由是读它的人不是界面而是后端 —— 每次 append 都要知道写满没写满，
而 `state.json` 是前端整份替换的文件，后端去读一份别人拥有的文件迟早会被覆盖；何况把这份日志、或这一份数据目录搬到
另一台机器时，跟着它走的应该是它自己的规矩。两个文件都没改过（或改成默认值）时根本不存在：「没选过」与「选成默认」
不该是两种状态。
...
搬家的顺序是**先复制再删**：逐个文件读出来、写到新目录、再读回来逐字节比一遍（`copyVerified`），名单里的
文件（连接 / 收藏 / 排法 / 状态 / `secret.key` / 日志轮转设置 / 数据生成上限，以及老版本那份根目录上的
`changelog.jsonl`）与三个文件夹（`.query` / `.mock` / `log`）都到位之后才改指针，最后删旧文件；删除是尽力而为，
...
名单写死在 `dataFiles`（连接配置、收藏、排法、状态、`secret.key`、日志轮转设置、数据生成上限，以及老版本那份
根目录上的 `changelog.jsonl`）与 `dataDirs`（`Queries` 那棵树即 `.query/`、自定义占位符那个 `.mock/`、日志那个
`log/` —— 一天一份，所以整个文件夹跟着搬），
...
界面有**三档语言**：English / 简体中文 / 繁體中文，默认英文，在**设置页 → Language** 里选 —— 命令条底下那条状态栏的右下角也有一格，只写当前那一档（`EN` / `简` / `繁`），点一下轮一档（Windows 上托盘的 `Interface language` 是第三个入口，见「托盘只有 Windows 有」），三个入口改的是同一个偏好、走的是同一条路（`setUiLanguage`：切文案、重写开着的窗口标题、落盘）。语言是**存下来的偏好**
而语言切换又是**当场生效**的，所以它在 `state.json` 里占一个字段（`language`），跟主题、代码窗口的默认语言一起由
`saveUi` 整份写出去；存档里没有这个字段、或者写着一个这个版本不认识的 id，就回到英文（换语言不会把界面卡在
一个读不出来的值上）。一句话：**语言不进 store**。它归 `lib/i18n` 自己拿，`t()` 因此是个普通模块函数 ——
`api/client.ts`、`store/appStore.ts` 这些没有组件的地方照样能读文案；需要重画的组件用 `useLanguage()` 订阅一次，
所以切语言时重渲染的是**读了文案的那些组件**，不是整个应用（重挂根节点会把查询窗口里没保存的 SQL 一起抹掉）。

**文案是一张表，一行一句**：`frontend/src/lib/i18n/messages/<区域>.ts`，键是 `<区域>.<片段>`，值是
`[英文, 简体, 繁体]` —— 英文那一列是**代码当初写下的话**，另两列是译文，**空着就回落英文**（没用过的列不用编）。
键按区域分文件而不是挤成一个大表，是为了让一句文案和写它的组件对得上：`node scripts/i18n.mjs list` 按完成度列出
每个区域，`prune` 找出没人读的键（改文案改掉的残行），`export` 吐出还没翻译的、`fill` 把译好的灌回去。键里只有
`<区域>` 前缀和一小段英文摘要，**中文不进键**（从中文反推键会撞车）。文案读法有三档，按句子里**夹着什么**分：
`t('key', { name })` 是普通一句，`{name}` 这种占位符由 `t` 填；`tn('key', count)` 管单复数，消息里用 `|` 隔开两种
形态（`'{n} row|{n} rows'`，中文两边一样，照写），数还带千分位时用 `{ n: count.toLocaleString() }` 覆盖填进去的 `{n}`；`tr('key', { sample: <span…/> })`
给**句子里夹着标记**的那种话用 —— `写 @@ 表示一个真正的 @` 这种提示不能由几段 `t()` 拼起来，拼出来的中文永远
是英文的语序（那正是 i18n 要治的病），所以整句是一条消息，值当参数插进去。而**模块级常量表里的词**（比如
连接对话框那几个标签）存的是**键**而不是词：表格在 import 时建好一次，词要是那时就定下，页面就会永远停在被加载
那一刻的语言上。antd 自己的文案也跟着切（`antdLocaleOf`），窗口标题栏上的标签页标题是「当时语言」的成品，
所以切语言时 store 会把开着的窗口标题重写一遍（`retitleTabs`）；**数据名不翻译** —— 表名、库名、字段名、SQL、
用户写的 mock 模板都是数据。同样是数据、因此不动的还有两份表：取色器目录里的占位符说明（`lib/mock/catalog.ts`）
与代码生成里那 18 种语言的名字（`lib/codegen/languages.ts`）—— 前者是对着 mock.js 写的说明，后者是别人家的语言名。

**后端那一半（Go）的文案仍是英文**，这是已知的后续工作：界面里凡是由后端拼出来的话（引擎自己的报错、变更日志里
那些说明）现在原样显示英文；前端这一半已经全部走 `t()`（可以用 `node scripts/i18n.mjs list` 看到 100% 的那一行）。
唯一的例外是 **Windows 托盘菜单**：它由 Go 画、也只由 Go 报回选中了哪一行，走不了前端的文案表，所以那九行在
`tray.go` 里按三档语言各写了一遍（`trayWordsByLanguage`，见「托盘只有 Windows 有」），用词与设置页对齐。
...
菜单文字、顺序、勾在哪儿与版本号只在 `tray.go` 一处（`trayMenuRows(prefs)`），`tray_windows.go` 只负责把它画成
Win32 的菜单：一行（`trayMenuRow`）是分隔线、命令、或「点开是一个子菜单的标题」三者之一，画的时候子菜单用
`MF_POPUP` 挂上去（子菜单归父菜单所有，所以只有最外那层在右键处理里手动销毁）。`tray_test.go` 断言这一列顶层行、
两条分隔线、「只有版本行不可点」、每一个 id 全局唯一（子菜单里的也算进这棵树）、三档语言各自的措辞，以及两个
子菜单里**恰好勾着一行**；Windows 上再加一组测试真的建一次菜单，用 `GetMenuString` / `GetMenuState` /
`GetSubMenu` 把每一层读回来，确认标签原样到达 Win32（中文也一样）、灰的灰、勾的勾、该有子菜单的确实有；
手写的 `NOTIFYICONDATAW` 与 `MSG` 也用测试钉住字段偏移量 —— `cbSize` 是 shell 判断结构体版本的唯一依据，
错一个字节就是一句没说出口的错误。

**菜单里那两档设置，偏好仍然只归前端一份。** 挑中「浅色」不是一条命令，而是**一次设置的变更**：Go 这一侧只把
选中的值当成事件发出去（`tray:theme` / `tray:language`），前端听到后走的就是设置页那条路 —— `setTheme` /
`setUiLanguage` 应用、`saveUi` 把 `state.json` 整份写出去。于是托盘**从不写状态文件**：同一份偏好的第二个写者，
是「切主题顺手把语言丢了」这类毛病的来源。反过来的方向也一样：菜单要勾哪一行，读的是**启动时与每次 `SaveState`
之后**存下的那份副本（`trayPrefsStore`），而不是每次右键再去读一遍刚被写了一半的文件 —— 一个说「浅色」而真值是
深色的菜单，是个没有收益的错答案。事件名两边各写一遍（Go 的 `trayThemeEvent` 与 `frontend/src/lib/tray.ts`），
`tray_test.go` 里那条测试会去读前端那个文件把两个名字钉住：拼错一个字母就是这样一种 bug —— 不报错、只在
Windows 上、菜单点了没反应。

`trayWordsByLanguage` 是 Go 侧唯一翻译过的文案（九行 × 三档语言）：菜单是原生的，画不了前端的文案表，而九行也
不值得配一套翻译系统，所以直接写在这儿，用词取设置页对同一件事的说法（`libTheme.*` 与
`settingsPane.display-theme` / `interface-language`），免得菜单与设置页把一个设置说成两样；三档语言名
（`English` / `简体中文` / `繁體中文`）是唯一不翻译的 —— 一个把「简体中文」写成英文的语言选择器，对最需要它的
人是没用的。语言不认识就回落英文，和界面本身一样。
...
**数据库导出**在 `internal/service/export_test.go` 里跑真 SQLite 会话：结构导出写出的是引擎给自己那张表的那句
`CREATE`（拿 `manager.Structure` 读回来的原文对一遍，而不是硬编码一段 DDL）、「结构和数据」两样都在、数据导出
**只写勾中的字段且按勾选顺序**（列清单与 CSV 表头都对过）、值按引擎的写法转义、CSV / JSON / JSONL 三种行格式
各对一遍（JSON 的数字要真的是数字、字段顺序不能乱）、`CancelExport` 之后报的是**已经写进去的那些**、
被停下的 JSON 也照样合上 `]`、读不出来的表进警告而 run 继续；另有一组「表单不该问出来的请求」被逐条拒掉
（没有 id / 没有会话 / 没有目标路径 / 没选表 / 空表名 / 认不出的档 / 认不出的格式 / CSV 配两张表 /
结构档配 CSV），以及勾了一个不存在的字段时**在文件建出来之前**就失败 —— 这条是给「dump 少了一列要到恢复时
才发现」兜底的。驱动这一层只额外测了 `sqlbase` 的字面量渲染（`literal_test.go`）：`StreamRows` 需要一个真的引擎才能跑，
而 `sqlbase` 不能反过来引驱动（会成环），所以它由上面那组端到端测试覆盖。

**运行 SQL 文件**也是两层。第一层连数据库都不需要：`internal/drivers/sqlutil/split_test.go` 盯住那个流式扫描器
（分号、引号里的 `;`、`--` 与 `/* */` 注释、`#` 注释、PostgreSQL 的 `$$` 与 `$tag$…$tag$` 整段、`DELIMITER //`
之后到 `//` 才切、注释与空白不变成语句、空脚本给空切片），流式那一支还逐条核对**读到哪里就报到哪里**
（byte 偏移与语句一一对应）。第二层 `internal/service/sqlfile_test.go` 在真 SQLite 上跑整条路：
读文件时说清楚里面有什么（几条语句、哪几条会丢结构或数据、只读连接会拒掉几条、`USE` 与认不出的语句进警告）、
跑起来按顺序一条不落、`StopOnError` 开着时停在失败那条而关着时接着往下、失败最多留 20 条并标出「还有」、
**每一条写语句都带着自己那一行数进变更日志**、`USE` 只改后面语句的目标而不发给引擎、Stop 之后停在**下一条**、
被 Stop 打断的那条不算失败、取消一个没在跑的 run 不报错、窗口不该问的请求（没有 id / 没有会话 / 没有文件 /
给的是文件夹）逐条被拒、MongoDB 那种「语句不是 SQL」的引擎回 `unsupported`。

**数据库比较**也分两层：`internal/drivers/sqlbase/diff_test.go` 拿手写的结构对出纯差异（没动过的表里找不到
一处不同、类型 / 可空 / 默认值 / 自增 / 注释 / 索引列各报一处、两边都是自增时不比默认值、主键不重复算成
索引差异、只有一边有的表写成 added / removed、名字大小写不同仍算同一张表），外加 `DesignOfStructure`
与 `KeepEngineCounters` 那两条：设计里除了主键索引什么索引都留着、`serial` 的默认值转成 identity、
目标自己的序列名不被左库的覆盖；`internal/service/compare_test.go` 则在真 SQLite 上端到端跑 —— 两个库上
跑不同的 DDL 再看差异与生成的脚本对不对得上、跳过的视图进警告、脚本按「先建、再改、最后删」排、
`DropExtra` 关着时右边多出来的表写进警告而不是被删、打开之后跑一遍真的把右边对齐了，
以及同一个库、不同引擎、只读的目标在动手之前就被拒掉。
...
托盘那一圈不需要桌面也能测：菜单内容按 `trayMenuRows(prefs)` 断言 —— 顶层那一列、id 唯一、只有版本行不可点、
三档语言各有一份措辞且都译过、两个子菜单里恰好勾着一行、设置行与它代表的值对得上、`state.json` 里读出来的偏好
（缺字段 / 写坏了 / 别的版本写的值都回落默认），Windows 上再加一组只在 Windows 跑的测试 —— 手写的
`NOTIFYICONDATAW` / `MSG` 字段偏移量与 `utf16Fill` 的截断 / 终止符、真的建一次托盘菜单再用 `GetMenuString` /
`GetMenuState` / `GetSubMenu` 把每一层读回来（标签原样 —— 中文也是、灰的灰、勾的勾、子菜单该有的有）、图标能从
可执行文件或系统默认图标里拿到。还有一条跨语言的：事件名会去读 `frontend/src/lib/tray.ts` 对一遍，拼错了就是
点了没反应、且只在 Windows 上。窗口那半边（关窗进托盘、点图标回来、菜单里的 `Quit` 干净退出）是脚本化的手工
烟测：用 `PostMessage` 把 shell 会发的消息发给应用自己的窗口，看进程还在不在、窗口可见不可见，不放进 CI。
...
- [x] Phase 1.5：
  ...
  - [x] 变更日志：图标栏的「变更日志」一页看这个程序跑过的每一句写语句（时间 / 连接 / 库 / 表 / 影响行数 / 语句），存在数据目录里（`<数据目录>/log/<yyyymmdd>.log`，一天一个文件），没连库也能打开；写满了就整份留档成 `log/20260214-1.log`（阈值可设，默认 2000 条），历史日志在同一个窗口里选着看
  - [x] 数据生成：按 mock.js 模板填表 —— 每列一段模板（可勾选、自增列默认不勾）、一个连接一个窗口、占位符取色器与自定义占位符（`<数据目录>/.mock`）
  - [x] 页面栏：连接 / 数据生成 / 比较 / 变更日志 / 设置五页各占一页，切走不丢状态（数据生成里写一半的 mock 仍在）；分隔条收起后用图标栏上的「连接」就能回来
  - [x] 连接排序与分组：整行拖动换位置，一层深的分组文件夹，排法存在 `layout.json` 并与实时配置对齐
  - [x] 界面语言：English / 简体中文 / 繁體中文 三档（默认英文，存在 `state.json`），文案表 + 抽取 / 导出 / 回填 / 清理脚本；后端 Go 的文案仍是英文，留下次
  - [x] 数据库导出：库 / schema 节点右键的三个入口（只要结构 / 结构和数据 / 只要数据）写 `.sql` 脚本或单表的 CSV / JSON / JSONL，字段可勾、进度与 Stop 都有 —— 整个 run 在后端一边读一边写，行不过桥
  - [x] 运行 SQL 文件：库节点右键把磁盘上的一份 `.sql` 交给后端跑 —— 文件不过桥（进度按字节算）、一条语句一次调用，所以每句写语句都带着自己的影响行数进日志；读过之后先摆出里面有什么（会丢数据的标红、只读连接会拒几条、`USE` 与认不出的语句进警告），跑的时候有进度与 Stop，停下时报出「跑了几条、失败几条、改了几行」与失败在文件里是第几条 —— 并且直说**没有事务包着**，停下不会回滚
  - [x] 托盘（Windows）里切显示模式与界面语言：窗口收进托盘之后菜单是唯一够得着的地方，所以两档设置就在菜单里（当前那档打勾，整个菜单按当前语言写）；挑中走的是和设置页同一条路 —— Go 只把选中的值当事件发出去，前端应用并落盘，`state.json` 仍只有一个写者
...
- [x] Phase 3.5：数据库比较：两个同引擎的库摆在一起看差异，差异本身就是同步脚本（先建、再改、最后删），跑完记变更日志
  - [x] 写操作先确认再执行，跑完记下改了多少行：程序自己生成的每一条写语句（建表 / 改结构 / 清空 / 删除 / 复制 / 改行删行 / 生成数据 / 同步脚本 / 新建库）都在确认框里先摆出**后端渲染的那条语句**（删多行就摆出每一条），确认才跑；执行的那一层把语句与引擎报的**受影响行数**记进变更日志，日志改成**一天一个文件**（`<数据目录>/log/<yyyymmdd>.log`，满 2000 条整份留档成 `log/<yyyymmdd>-N.log`，老版本的 `changelog.jsonl` 仍读得到）
- [ ] Phase 4：SSH 隧道、表格数据导入（CSV / Excel）、插件式扩展
