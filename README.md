...
- **外观**：Navicat 式窗口骨架（icon-over-label 命令条 + 最左那条五页图标栏 + 连接树 + 标签页工作区 + 状态栏）、明暗主题、品牌绿 `#36ab60`、可拖拽分栏、紧凑的表格与状态栏 —— 表格的表头**固定不动**：数据网格、对象列表、设计器的字段表都一样，纵向滚动时表头留在容器顶上，横向滚动也带不走它（钉住的列仍旧钉在原处）。主题有三档：`Light` / `Dark` / `System`，在设置页的 **Appearance** 里选，Windows 上托盘菜单的 **`Display theme`** 是同一个偏好的另一个入口（见「托盘只有 Windows 有」）（跟随系统时它会写成 `system (dark)`，把当前系统给的那一档一起说出来）；选**跟随系统**时窗口真的跟着操作系统走 —— 操作系统在运行期间切深浅色，界面当场就变，不用重启也不用再点一次。状态栏右下角那一格是**浅色 / 深色两档的开关**，写的永远是**屏幕上正在用的那一档**（不是存下来的偏好：`System` 不是一个能被点亮的状态，而是「为什么现在是这一档」），点一下平移到另一档；正在跟随系统时，提示里会说明这一点并告诉你会固定成哪一档（`Light` ⇄ `Dark`），所以「跟随系统」不会变成一个够不着的偏好 —— 托盘与设置页照旧管它。紧挨着它的是**界面语言的 `EN` / `简` / `繁` 三格**（按一下当场换，当前那一档是品牌色，完整名字在悬停提示里，各自写着它自己的语言）—— 这是**不走设置页**换语言的地方，和托盘的 `Interface language`、设置页的 `Language` 是同一个偏好的三个入口。命令条上目前只有 **Connection**、**Open**、**Close**、**New Query**、**Refresh**、**Table** 与 **View** 是活的，其余按钮保持原来的位置但禁用并在提示里说明 —— 摆着的空位比消失的按钮更好认。切**页面**的命令不在命令条上，而在最左边那条图标栏里（见「页面栏与五个页面」）：命令条只管**作用在当前连接上的事**。
...
界面有**三档语言**：English / 简体中文 / 繁體中文，默认英文，在**设置页 → Language** 里选 —— 命令条底下那条状态栏的右下角也摆着 `EN` / `简` / `繁` 三格（Windows 上托盘的 `Interface language` 是第三个入口，见「托盘只有 Windows 有」），三个入口改的是同一个偏好、走的是同一条路（`setUiLanguage`：切文案、重写开着的窗口标题、落盘）。语言是**存下来的偏好**
