...
│   │   ├── sqlbase/          # 通用 database/sql 实现：连接池、分页、脚本执行、DDL（design.go 的规划器 + copy.go 的复制表）、行变更（含 literal.go 的语句预览字面量）、运行情况外壳
...
| 一个对象（表 / 视图…） | Open data / Design table（给不出设计器的引擎是 Open fields）/ Edit DDL…（关系型才有）/ Generate code…（有字段的对象才有）/ Data generation…（表 + 能 INSERT 的引擎才有）/ **Duplicate table → Structure only · Structure and data**（表 + 能建表的引擎才有）/ Copy name |
