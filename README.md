...
- **外观**：Navicat 式窗口骨架（icon-over-label 命令条 + 连接树 + 标签页工作区 + 状态栏）、明暗主题、品牌绿 `#36ab60`、可拖拽分栏、紧凑的表格与状态栏。命令条上目前只有 **Connection**、**Close** 与 **Refresh** 是活的（Close 在连接树里选中一条已开启的连接后亮起，点一下就是断开它），其余按钮保持原来的位置但禁用并在提示里说明 —— 摆着的空位比消失的按钮更好认。
