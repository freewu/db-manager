...
- **数据网格**：分页、服务端排序、服务端过滤（14 种操作符）、**列宽可拖动**（表头右边缘按住左右拖，拖过一次之后宽度就按拖出来的算、不再随窗口拉伸）、长文本悬浮预览、多选、行详情抽屉（JSON / INSERT 预览）。
...
**为什么要「一次全冻住」而不是只定住被拖的那一列**：表格铺满窗格时，每列的实际宽度是浏览器按内容分配出来的 ——
你按住的只是其中一列，但拖宽它会让别的列当场被挤窄，光标下那条边于是按另一种速度移动，手感是「打滑」；
先把当前这一刻的宽度照单全收、再只动一列，拖动才与光标的位移严格相等（而且冻结的那一刻屏幕上看不出任何变化，
因为量到的宽度之和本来就等于窗格宽度）。冻结的宽度是**量出来的**，不是编的：像对象列表的「名称 / 注释」或
设计器的「描述」这类本来就没有宽度的列，量到多少就是多少，不必替它们猜一个默认值（只有「冻结之后才冒出来的
新列」才落到 160px 这个兜底）。

**拖过之后的表格不再拉伸**：宽度既然是拖出来的，就不该再随窗口变化 —— 一张被拖窄的表格右边空出一条，
正是「这一列就是这么宽」的意思（Excel / Navicat 就是这么回事）。实现见
