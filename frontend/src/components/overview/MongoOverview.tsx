 ...
 *
 * The per-database table is `dbStats` for every database the user can see. It
 * comes last on purpose: it is the only part that grows with the server's
 * content rather than its activity.
 */
export function MongoOverview({ overview }: { overview: ServerOverview }) {
  const mongo = overview.mongodb
  if (!mongo) return null

  return (
    <>
      {mongo.groups.map((group) => (
        <MetricGroup key={group.title} group={group} />
      ))}

      {mongo.databases ? <DataTable table={mongo.databases} /> : null}

      <Typography.Text type="secondary" style={{ fontSize: 12 }}>
        Counters come from <span className="mono">serverStatus</span> and reset when mongod
        restarts; the table below reads <span className="mono">dbStats</span> per database and
