...
export const settingsPane = {
  ...
  'settingsPane.file-log': [
    'The change log, one file per day',
    '变更日志，一天一个文件',
    '變更日誌，一天一個檔案',
  ],
  'settingsPane.file-datagen-json': [
    'The most rows one data generation run may write',
    '数据生成一轮最多能写多少行',
    '資料產生一輪最多能寫多少列',
  ],
  'settingsPane.file-changelog-jsonl': [
    'The change log written by an older build, one statement per line',
    '老版本写的变更日志，每行一条',
    '舊版本寫的變更日誌，每行一條',
  ],
  ...
  'settingsPane.archived-change-log-by-an-older-build': [
    'An archived change log of an older build',
    '老版本留下的变更日志留档',
    '舊版本留下的變更日誌封存',
  ],
  ...
  'settingsPane.change-log-hint': [
    "Statements go into the log folder, into the file for the day they ran on — <date>.log — until it holds this many; then it is moved aside whole as <date>-<n>.log and a new file for the same day is started. Nothing is ever dropped: the archives keep every statement, and the Change log window lists them.",
    '语句写进 log 文件夹里、它跑的那一天的那份文件 —— <date>.log —— 写满这么多条后，整个文件会被改名为 <date>-<n>.log 挪到一边，再为同一天新建一个文件。不会有任何丢弃：归档里保留每一条语句，「变更日志」窗口会列出它们。',
    '語句寫進 log 資料夾裡、它跑的那一天的那份檔案 —— <date>.log —— 寫滿這麼多條後，整個檔案會被改名為 <date>-<n>.log 挪到一旁，再為同一天新建一個檔案。不會有任何丟棄：封存裡保留每一條語句，「變更日誌」視窗會列出它們。',
  ],
